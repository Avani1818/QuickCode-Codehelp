...
export function activate(context: vscode.ExtensionContext) {
    ...
    const selectLanguage = async (): Promise<string | undefined> => {
...
function extractDataAfterEqual(input: string): string[] {
    ...
    while (currentPos < input.length) {
        ...
        if (commaSpacePos !== -1) {
            ...
            if (segment) dataArray.push(segment); // Add only non-empty segments
...
function parseExtractedData(input: string): any[] {
...
async function fetchTestCasesFromLeetCode(problemURL: string) {
...
function readTestCases(folderPath: string): { input: any; output: string }[] {
    try {
        const testCases: { input: any; output: string }[] = [];
...
function runPythonSolution(solutionPath: string, testCases: { input: any; output: string }[]): Promise<any[]> {
    return Promise.all(
        testCases.map(testCase =>
            new Promise(resolve => {
                const process = exec(`python "${solutionPath}"`, (error, stdout, stderr) => {
...
function runCppSolution(solutionPath: string, testCases: { input: any; output: string }[]): Promise<any[]> {
    const compiledPath = path.join(path.dirname(solutionPath), 'solution.exe');

    return new Promise((resolve, reject) => {
        // Compile the C++ solution
        exec(`g++ "${solutionPath}" -o "${compiledPath}"`, (compileError, stdout, stderr) => {
            if (compileError) {
            ...
            Promise.all(
                testCases.map(testCase =>
                    new Promise(resolveTestCase => {
                        const process = exec(`"${compiledPath}"`, (error, stdout, stderr) => {
                            const actualOutput = error ? stderr.trim() : stdout.trim();
                            resolveTestCase({
                                input: testCase.input,
                                expected: testCase.output,
...
function displayResults(results: any[]): void {
    ...
    results.forEach((result, index) => {
        terminal.sendText(
            `echo "Test Case ${index + 1}: ${result.passed ? 'Passed ✅' : 'Failed ❌'}"`
        );
        if (!result.passed) {
    ...
    vscode.window.showInformationMessage(
        'Test case execution completed. Check the "Test Results" terminal for details.'
    );
}
